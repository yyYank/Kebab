...
class RemoteDriverOperation {
     ...
     * is returned.
     */
    fun getAugmentedDriver(driver: WebDriver): WebDriver = if (isRemoteDriverAvailable()) {
        softLoadRemoteDriverClass(this.classLoader, "Augmenter")?.newInstance()!!//augment(driver)
    } else {
        driver
    }

    fun softLoadRemoteDriverClass(classLoader: ClassLoader?, classname: String): Class<WebDriver>? {
        try {
            return classLoader?.loadClass("org.openqa.selenium.remote.$classname") as Class<WebDriver>
